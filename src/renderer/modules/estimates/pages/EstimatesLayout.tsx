...
export default function EstimatesLayout() {
  ...
  const onKanban = /^\/estimates\/boards\/[^/]+/.test(location.pathname);

  const tabs = limitedUser
    ? []
    : [
        { to: ROUTES.ESTIMATES.INDEX, label: 'Estimates', icon: FileSpreadsheet, exact: true },
        { to: ROUTES.ESTIMATES.PROJECTS, label: 'Projects', icon: FolderKanban, exact: true },
        { to: ROUTES.ESTIMATES.BOARDS, label: 'Project boards', icon: Kanban, exact: true },
        ...(showFullEstimates ? [
          { to: ROUTES.ESTIMATES.INSIGHTS, label: 'Insights', icon: TrendingUp, exact: true },
          { to: ROUTES.ESTIMATES.TEMPLATES, label: 'Templates', icon: LayoutTemplate, exact: true },
        ] : []),
      ];
