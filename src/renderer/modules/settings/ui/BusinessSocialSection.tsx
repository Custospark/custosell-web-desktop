...
import { BusinessSectionCard, inputClass, labelClass } from './businessSettingsFormShared';
...
export function BusinessSocialSection() {
  ...
  const canAdd = platform.trim().length > 0 && url.trim().length > 0;
  ...
  const handleAdd = () => {
    ...
    upsert.mutate(
      { data: { platform: platform.trim(), url: normalizeUrl(url) } },
      {
        onSuccess: () => {
          setPlatform('');
          setUrl('');
    ...
    >
      ...
      <div className="rounded-lg border border-dashed border-gray-300 p-3">
        ...
        <p className="mt-2 flex items-center gap-1.5 text-xs text-gray-500">
          ...
          Any platform name works — adding the same platform again updates its URL.
        </p>
      </div>
