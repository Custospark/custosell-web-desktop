import { defineConfig } from 'vitest/config';

export default defineConfig({
  test: {
    environment: 'happy-dom',
    include: [
      'src/renderer/modules/pipeline/api/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/shifts/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/notes/**/*.test.{ts,tsx}',
      'src/renderer/modules/sales/api/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/storefront/cart/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/invoices/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/expenses/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/pipeline/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/documents/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/modules/platform/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/app/store/offline/**/*.test.{ts,tsx}',
      'src/renderer/app/store/slices/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/shared/utils/__tests__/**/*.test.{ts,tsx}',
      'src/renderer/shared/components/payments/__tests__/**/*.test.{ts,tsx}',
    ],
    restoreMocks: true,
    clearMocks: true,
    testTimeout: 60000,
    hookTimeout: 60000,
    fileParallelism: false,
    maxWorkers: 1,
  },
});