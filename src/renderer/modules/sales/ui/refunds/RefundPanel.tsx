          ...
          <div className="space-y-5">
            ...
            <div className="bg-gray-50 rounded-xl p-4">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0">
                  <p className="text-sm font-bold text-gray-900 uppercase">{user?.business_name?.toUpperCase() || 'CUSTOSELL'}</p>
              ...
              <div className="mt-3 pt-3 border-t border-gray-200 grid grid-cols-2 sm:grid-cols-3 gap-x-4 gap-y-3 text-xs">
                <div>
                  <p className="text-gray-400">Date</p>
                  <p className="text-gray-700 font-medium">{new Date(selectedSale.sale_date).toLocaleDateString()} {new Date(selectedSale.created_at).toLocaleTimeString()}</p>
                </div>
                <div>
                  <p className="text-gray-400">Payment</p>
                  <p className="text-gray-700 font-medium capitalize">{selectedSale.payment_method.replace('_', ' ')}</p>
                </div>
                {cashier && (
                  <div>
                    <p className="text-gray-400">Sales Person</p>
                    <p className="text-gray-700 font-medium">{cashier.name}</p>
                  </div>
                )}
                {customer && (
                  <div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2.5 px-1">
                <h4 className="text-sm font-semibold text-gray-700">Select items to refund</h4>
                {(selectedSale.sale_items || []).some((i) => i.quantity - i.refunded_quantity > 0) && (
                  <span className="text-xs text-gray-400">Adjust quantity to refund</span>
                )}
              </div>
