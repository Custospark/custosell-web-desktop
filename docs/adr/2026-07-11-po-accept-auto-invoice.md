...
3. **Payments hub** — Record payment, receipt history, and PDF stay under **Invoices**. PO lists only deep-link (`?po=&invoice=&focus=payments`).
4. **Delete** — `DELETE /purchase-orders/{id}` allowed for **draft** (buyer only) and **rejected** (buyer or seller). Hard delete (avoids `po_number` unique collisions). Linked invoices block delete.
