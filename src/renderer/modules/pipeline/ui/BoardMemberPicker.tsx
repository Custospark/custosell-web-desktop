...
import type { BoardMemberInput, PipelineBoardMember } from '../api/pipelineTypes';
...
export default function BoardMemberPicker({ value, onChange, excludeUserId, className }: BoardMemberPickerProps) {
  ...
  const addMember = (userId: number) => {
    onChange([...value, { user_id: userId, role: 'editor' }]);
  };
  ...
  const nameFor = (userId: number) => staff.find((u) => u.id === userId)?.name ?? `User #${userId}`;
      ...
      {value.length > 0 && (
        <ul className="space-y-2">
          {value.map((member) => (
            <li
              key={member.user_id}
              className="flex items-center justify-between gap-2 rounded-lg border border-gray-200 bg-gray-50/80 px-3 py-2"
            >
              <span className="truncate text-sm font-medium text-gray-900">{nameFor(member.user_id)}</span>
...
export function membersFromBoard(members?: PipelineBoardMember[]): BoardMemberInput[] {
  return (members ?? []).map((m) => ({ user_id: m.user_id, role: m.role }));
}
