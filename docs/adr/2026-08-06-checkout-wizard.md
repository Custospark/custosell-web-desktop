...
- `ui/BillingControls.tsx` → became Step 2, kept as one coherent card (Customer → Payment Method → Installments → Amount Tendered → Discount → Totals → Complete Sale). Removed a duplicated itemised cart list that the previous two-column layout introduced; totals for the sale shown only here.
