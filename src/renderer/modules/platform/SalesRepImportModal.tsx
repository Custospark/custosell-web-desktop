...
interface SalesRepImportModalProps {
  open: boolean;
  onClose: () => void;
  onImported: () => void;
}

export default function SalesRepImportModal({ open, onClose, onImported }: SalesRepImportModalProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const { data } = await axiosInstance.post<ImportResult>(SALES_REPS.IMPORT, formData, {
      ...
      setResult(data);
      if (data.imported > 0) {
        showToast('success', `${data.imported} sales reps imported`);
        onImported();
      }
    } catch (err: unknown) {
  ...
  const handleDownloadTemplate = async () => {
    try {
      const { data } = await axiosInstance.get(SALES_REPS.IMPORT_TEMPLATE, {
        responseType: 'blob',
      });
      const url = window.URL.createObjectURL(new Blob([data]));
      const link = document.createElement('a');
      link.href = url;
      link.setAttribute('download', 'sales-rep-import-template.xlsx');
  ...
  return (
    <Modal isOpen={open} onClose={() => { reset(); onClose(); }} title="Import Sales Representatives" size="md">
      {!result ? (
        <div className="space-y-5">
          <p className="text-sm text-gray-500">
            Upload an Excel file (.xlsx, .xls, or .csv) with your sales rep data.
            <br />Max 20MB. Name, Email, and Commission Rate are required.
          </p>
