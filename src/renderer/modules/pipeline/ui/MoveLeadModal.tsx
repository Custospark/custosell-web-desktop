...
import { Kanban, ArrowRightLeft } from 'lucide-react';
...
export default function MoveLeadModal({
  open,
  lead,
  ...
  const handleMove = async () => {
    ...
    if (sameBoard) {
      const stage = stages.find((s) => s.id === resolvedStageId);
      const existingLeads = stage?.leads ?? [];
      const position = existingLeads.length > 0
        ? existingLeads[existingLeads.length - 1].position + 1
        : 1;
  ...
  const isPending = moveLead.isPending || createLead.isPending || deleteLead.isPending || pending;

  const itemLabel = workspace === 'estimates' || lead?.card_type === 'card' ? 'card' : 'lead';

  return (
    <Modal isOpen={open} onClose={handleClose} title={`Move ${itemLabel}`} size="md">
      <div className="space-y-5">
        ...
        <div className="flex justify-end gap-2 border-t border-gray-100 pt-4">
          ...
          >
            ...
            {sameBoard ? `Move ${itemLabel}` : `Move & remove from current board`}
          </Button>
        </div>
      </div>
    </Modal>
