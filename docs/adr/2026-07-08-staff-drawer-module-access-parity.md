...
1. **Owner modules from Staff update are persisted.** Backend `UserService::validateModulesUpdate` no longer strips `modules` for the business owner. Owner payloads use `normalizeOwnerModules()` (Settings always included). After an owner module update via staff PUT, staff catalogs are clamped the same way as profile Module Access saves.
...
| Owner removes a module staff still have | `clampStaffModulesAfterOwnerUpdate` trims staff to owner catalog |
