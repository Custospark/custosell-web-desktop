...
function LineDiscountInput({ item, onDiscountChange, fluid = false }: { item: CartItem; onDiscountChange: SaleCartTableProps['onDiscountChange']; fluid?: boolean }) {
  return (
    <div className={fluid ? 'flex-1 min-w-0' : 'w-20 mx-auto'}>
      <MoneyInput
  ...
  return (
    <div className="flex items-center justify-center gap-1">
      <button title="Decrease quantity" type="button" onClick={() => onDecreaseQty(item)}
        className="w-7 h-7 rounded-full border-2 border-red-400 hover:border-red-500 hover:bg-red-50 text-red-500 hover:text-red-700 transition-all flex items-center justify-center">
        <Minus className="w-3.5 h-3.5" />
      </button>
      <span title="Click to edit quantity"
        className="w-8 text-center text-sm font-semibold text-gray-900 tabular-nums cursor-pointer hover:text-blue-600 transition-colors flex flex-col items-center leading-tight"
        onClick={() => onEditQty(item, product)}>
        <span>{item.quantity}</span>
        <span className="inline-flex items-center gap-0.5 text-[8px] font-medium text-blue-500 leading-none">
...
/** In-stock hint next to a cart line name — hidden for services. */
function StockHint({ product }: { product: Product | undefined }) {
  if (!product || !tracksStock(product)) return null;
  const qty = product.stock_quantity;
  return (
    <span className={`inline-block ml-1.5 text-[10px] font-medium whitespace-nowrap ${qty > 0 ? 'text-green-600' : 'text-red-500'}`}>
      ...
      <div className="overflow-x-auto">
        <table className="w-full">
          <thead className="bg-gray-50">
            <tr>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider">#</th>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider">Item</th>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider">Price</th>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider">Line discount</th>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider">Qty</th>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider">Subtotal</th>
              <th className="px-3 py-2 text-center text-[11px] font-medium text-gray-500 uppercase tracking-wider"></th>
            </tr>
          </thead>
            ...
            {items.map((item, idx) => {
              ...
              return (
                <tr key={`${item.product_id}-${item.price_tier}`} className="hover:bg-gray-50 transition-colors">
                  <td className="px-2 py-1.5 text-center text-sm text-gray-400">{idx + 1}</td>
                  <td className="px-3 py-1.5">
                    <span className="text-sm font-medium text-gray-800 truncate block max-w-[150px] sm:max-w-[180px]">{item.name}</span>
                    <div className="flex items-center mt-0.5">
                      <StockHint product={product} />
                      <div className="ml-1"><TierPicker item={item} onTierChange={onTierChange} /></div>
                    </div>
                  </td>
                  ...
                  <td className="px-3 py-1.5 text-center">
                    <button title="Remove item" type="button" onClick={() => onRemove(item)}
                      className="w-7 h-7 hover:border-red-500 hover:bg-red-50 text-red-500 hover:text-red-700 transition-all flex items-center justify-center">
          ...
          <tfoot className="bg-gray-50 border-t border-gray-200">
            <tr>
              <td colSpan={5} className="px-3 py-2 text-right font-semibold text-gray-900">Subtotal:</td>
              <td className="px-3 py-2 text-right"><span className="text-base font-bold text-gray-900">{formatCurrency(subtotal)}</span></td>
              <td></td>
            </tr>
          </tfoot>
        </table>
  ...
  return (
    <div className="sm:hidden space-y-1.5">
      {items.map((item) => {
        const product = products?.find((p) => p.id === item.product_id);
        return (
          <div key={`${item.product_id}-${item.price_tier}`} className="rounded-lg border border-gray-200 bg-white p-2.5 shadow-sm">
            <div className="flex items-start justify-between gap-2">
              ...
              <button title="Remove item" type="button" onClick={() => onRemove(item)}
                className="w-7 h-7 shrink-0 hover:border-red-500 hover:bg-red-50 text-red-500 hover:text-red-700 transition-all flex items-center justify-center">
                <Trash className="w-4 h-4" />
              </button>
            </div>
