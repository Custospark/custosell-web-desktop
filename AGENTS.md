...
| 15 | **Deployment guardrails (READ BEFORE any deploy).** Before writing any deployment plan or touching staging/production, read `../DEPLOYMENT.md` in full. Every deploy requires a written deployment plan approved by Oscar first (todo list + rollback + verification). **Never** wipe any docroot before creating a rollback backup; always `cp -rT` (never `*` - it drops `.htaccess`); verify every asset referenced by `index.html` exists and served JS is `javascript` MIME (not `text/html`) before declaring success. The web build ships via `Backend/public/<target>`; commit only the exact build path, never `git add -A` on the shared backend repo. |
