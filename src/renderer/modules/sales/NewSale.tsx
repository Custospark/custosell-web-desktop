  ...
  return (
    <div className="h-full flex flex-col">
      ...
      <div className="mb-6 pb-4 border-b border-gray-200">
        ...
        <p className="text-sm text-gray-500">Search products and complete sale</p>
      </div>
      ...
      <div className="flex gap-6 flex-1 overflow-hidden">
                        ...
                        <table className="w-full">
                          <thead className="bg-gray-50 sticky top-0">
                            <tr>
                              ...
                              <th className="px-4 py-2 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-10">Action</th>
                            </tr>
                          </thead>
                              ...
                              <tr key={p.id} className="hover:bg-blue-50 cursor-pointer transition-colors" onMouseDown={() => addItem(p.id, p.name, parseFloat(p.unit_price))}>
                                <td className="px-4 py-3">
                                  <div className="flex items-center gap-3">
                                    <div className="p-1.5 rounded-lg bg-gray-100 text-gray-500"><Package className="w-4 h-4" /></div>
                ...
                <table className="w-full">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase tracking-wider w-10">Action</th>
                    </tr>
                  </thead>
                  <tbody className="bg-white divide-y divide-gray-100">
                    {cartItems.map((item, idx) => (
                      <tr key={item.product_id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <td className="px-4 py-3"><span className="text-sm font-medium text-gray-800">{item.name}</span></td>
                        <td className="px-4 py-3 text-right"><span className="text-sm text-gray-600">{formatCurrency(item.unit_price)}</span></td>
                        <td className="px-4 py-3">
        ...
        <div className="w-96 flex flex-col bg-white rounded-xl border border-gray-200">
          ...
          <div className="flex-1 p-5 space-y-5 overflow-y-auto">
              ...
              <div>
                ...
                {amountTendered > 0 && amountTendered < subtotal && (
                  <p className="text-xs text-amber-600 mt-2 flex items-center gap-1">
                    ⚠️ Insufficient — short by {formatCurrency(subtotal - amountTendered)}
                  </p>
                )}
              </div>
            )}

            {/* Totals + Change Summary */}
            <div className="p-4 bg-gray-50 rounded-xl space-y-2">
              <div className="flex justify-between items-center">
                <span className="text-gray-600">Total Amount</span>
                <span className="font-bold text-gray-900 text-lg tabular-nums">{formatCurrency(subtotal)}</span>
              </div>
              {paymentMethod === 'cash' && amountTendered > 0 && (
                <>
                  ...
                  <div className="flex justify-between items-center border-t border-gray-200 pt-2">
                    ...
                    <span className="font-bold text-green-600 text-lg tabular-nums">{formatCurrency(changeDue)}</span>
                  </div>
                </>
              )}
            </div>
            ...
            <Button className="w-full h-12 text-base font-semibold" onClick={handleCompleteSale} loading={createSale.isPending}
