...
export interface Plan { id: number; name: string; slug: string; type?: string; description: string | null; price_monthly_usd: number; price_yearly_usd: number | null; onboarding_fee_usd: number | null; trial_days?: number | null; billing_cycle?: 'monthly' | 'yearly' | 'both'; features: Record<string, boolean>; limits: Record<string, number | null>; is_active: boolean; is_popular?: boolean; sort_order: number; metadata?: Record<string, unknown> | null; }
...
export interface Subscription { id: number; business_id: number; plan_id: number; status: 'active' | 'trialing' | 'trial' | 'past_due' | 'suspended' | 'cancelled' | 'expired'; billing_cycle?: 'monthly' | 'yearly'; starts_at: string; trial_ends_at: string | null; ends_at: string | null; cancelled_at: string | null; next_billing_date?: string | null; onboarding_fee_paid?: boolean; plan?: Plan; }
